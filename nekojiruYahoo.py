...
def get_new_item_embed(item):
    ...
    initprice = item['Initprice'][:-3] if item['Initprice'].endswith(
        ".00") else item['Initprice']
    currprice = item['Price'][:-3] if item['Price'].endswith(
        ".00") else item['Price']
    ...
    embed = {
        'title':
        '【{}】 - {}'.format(item['AuctionID'], item['Seller']['Id']),
        'description':
        '{}\n【[YAJ]({})】　【[Buyee]({})】\n'.format(
            item['Title'], item['AuctionItemUrl'],
            item['AuctionItemUrl'].replace("page.auctions.yahoo.co.jp/jp",
                                           "buyee.jp/item/yahoo")),
        'url':
        item['AuctionItemUrl'],
        'footer': {
            'text':
            '+{} | -{}　　{} {}　　　　　Ends at: '.format(rating['TotalGoodRating'],
...
def check_item(item):
    c.execute("SELECT AuctionID FROM auctions WHERE AuctionID=?",
              (item['AuctionID'], ))
    if c.fetchone():
        time_diff = dateutil.parser.parse(item['EndTime']) - datetime.now(
            timezone.utc)
...
def main():
    ...
    while keep_reading:
        ...
        next_id = int(attr['firstResultPosition']) - 1 + int(
            attr['totalResultsReturned'])
